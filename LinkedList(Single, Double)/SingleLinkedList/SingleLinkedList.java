...
public class SingleLinkedList implements LinkedList{
	...
	public boolean insert(Object data) {
		...
		// head가 null이면
		if(this.head == null) {
			// head에 새로운 노드를 추가하고 size에 1을 추가한다.
		...
		// head가 null이 아니면
			...
			Node tmp = this.head; //tmp는 List를 탐색하는데 사용될 Node타입 변수이다.
	...
	public boolean delete(Object target) {
		...
		return false; // 모든 Node를 탐색하고도 해당 data값을 가진 Node가 없다면 false를 반환한다.
	}
	...
	public boolean contains(Object searchingData) {
		...
		// serachingData와 동일한 data값을 가진 Node가 없다면 false를 반환한다.
		return false;
	}
